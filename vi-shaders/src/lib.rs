...
pub fn main_cs(
    ...
    #[spirv(storage_buffer, descriptor_set = 0, binding = 1)] results: &mut [u8],
) {
    let index:u32 = id.x;
    // results[index as usize] = inputs[0] as u8;
    // return;
    // let mut w = [0u32; 80];
    let mut w = [0u32; 16];
    // w.copy_from_slice(&inputs[0..16]);
    for i in 0..16 {
        w[i] = idx(inputs, i);
    }
    ...
    let my_nonce = nonce_start.wrapping_add(index as u64);
    w[0] = (my_nonce >> 32) as u32;
    w[1] = my_nonce as u32;
    // assert!(nonce_start == 0);
    // assert!(h0 == 0x67452301);
    // assert!(h1 == 0xEFCDAB89);
    // assert!(h2 == 0x98BADCFE);
    // assert!(h3 == 0x10325476);
    // assert!(h4 == 0xC3D2E1F0);
    // assert!(w[0] == 0);
    ...
    // let result_h0 = h0.wrapping_add(a);



    // if the resulting hash, in hex, starts with "FACE", set my result to 1
    results[index as usize] = ((result_h0 >> 10) == 0x0000_0000) as u8;
    // results[index as usize] = (((result_h0 >> 24) as u8) == 0xFAu8) as u8;
    // results[index as usize] = (result_h0 > 1) as u8;
}
